  ...
  std::vector<std::vector<TransactionJson>> parallelize(BlockJson &block) {
    ...
    for(int i = 0; i < num_tx; ++i) {
      ...
      if(user_related_bin >= 0 && recv_related_bin >= 0) {
        ...
        std::copy(independ_user[recv_related_bin].begin(),independ_user[recv_related_bin].end(),std::back_inserter(independ_user[user_related_bin]));
        independ_user[recv_related_bin].clear();

        std::copy(independ_recv[recv_related_bin].begin(),independ_recv[recv_related_bin].end(),std::back_inserter(independ_recv[user_related_bin]));
        independ_recv[user_related_bin].clear();

        std::copy(independ_txs[recv_related_bin].begin(),independ_txs[recv_related_bin].end(),std::back_inserter(independ_txs[user_related_bin]));
