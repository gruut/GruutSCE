...
private:
  ...
  std::vector<hash_t> m_tx_merkle_tree;
...
public:
  ...
  bool initialize(const nlohmann::json &msg_block) {
    ...
    m_block_pub_time = static_cast<timestamp_t>(stoll(JsonTool::get<string>(msg_block, "btime").value()));

    m_block_id = JsonTool::get<string>(msg_block["block"], "id").value();
    m_block_time = static_cast<timestamp_t>(stoll(JsonTool::get<string>(msg_block["block"], "time").value()));
    m_world_id = JsonTool::get<string>(msg_block["block"], "world").value();
    m_chain_id = JsonTool::get<string>(msg_block["block"], "chain").value();
    m_block_height = stoi(JsonTool::get<string>(msg_block["block"], "height").value());
    m_block_prev_id = JsonTool::get<string>(msg_block["block"], "previd").value();
    m_block_prev_sig = JsonTool::get<string>(msg_block["block"], "link").value();
    m_block_hash = JsonTool::get<string>(msg_block["block"], "hash").value();

    setTxaggs(msg_block["tx"]);
    setTransaction(m_txaggs); // txagg에서 트랜잭션 정보를 추출

    m_tx_root = JsonTool::get<string>(msg_block["state"], "txroot").value();
    m_us_state_root = JsonTool::get<string>(msg_block["state"], "usroot").value();
    m_cs_state_root = JsonTool::get<string>(msg_block["state"], "csroot").value();
    m_sg_root = JsonTool::get<string>(msg_block["state"], "sgroot").value();

    if (!setSigners(msg_block["signer"]))
      return false;
    setUserCerts(msg_block["certificate"]);

    m_block_prod_info.signer_id = JsonTool::get<string>(msg_block["producer"], "id").value();
    m_block_prod_info.signer_sig = JsonTool::get<string>(msg_block["producer"], "sig").value();
  ...
  bool setSigners(const nlohmann::json &signers) {
    ...
    for (auto &each_signer : signers) {
      m_signers.emplace_back(JsonTool::get<string>(each_signer, "id").value(), JsonTool::get<string>(each_signer, "sig").value());
    }
    return true;
  }
  ...
  bool setUserCerts(const nlohmann::json &certificates) {
    ...
    for (auto &each_cert : certificates) {
      ...
      auto id = JsonTool::get<string>(each_cert, "id");
      if(!id.has_value()) {
        continue;
      }
